...
#ifndef kXW_SYNTH_REGULAR_EXPRESSION
#define kXW_SYNTH_REGULAR_EXPRESSION(_regex_, _expression_) \
- (BOOL)_regex_ { \
NSPredicate *regextest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", _expression_]; \
return [regextest evaluateWithObject:self]; \
}
#endif

@implementation NSString (XWValidator)

- (BOOL)xwIsPureChinese {
    for (NSUInteger i = 0; i < self.length; i++) {
        int characterInt = [self characterAtIndex:i];
        if (!(characterInt > 0x4e00 && characterInt < 0x9fff)) {
            return NO;
        }
    }
    return YES;
...
{
    NSLog(@"%@",[self substringFromIndex:self.length-1]);
...
{
    return self.length > 0 && self.length <= 8;
}

kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidQRCode, @"^[0-9a-zA-Z]{8}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidPhoneNumber, @"^1[3-9]\\d{9}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidCarNumber, @"^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z_0-9]{5}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidRealName, @"^([\u4e00-\u9fa5]){2,7}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsVaildLandlineNum, @"^([0-9]{3,4})[0-9]{7,8}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidEmail, @"^(\\w)+(\\.\\w+)*@(\\w)+((\\.\\w+)+)$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidPassword, @"^([A-Z]|[a-z]|[0-9]|[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~@#%……&*]){6,20}$")
//kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidNickName, @"^[\u4e00-\u9fff]{4,8}$")
kXW_SYNTH_REGULAR_EXPRESSION(xwIsValidCash, @"^[0-9]*$")
@end
