...
- (void)createCircles
{
    for (int i = 0; i < 9; i++)
    {
        ...
        XXLockCircle *circle = [[XXLockCircle alloc] initWithDiameter:self.circleMargin * 3];
...
- (void)showErrorPasscode:(NSString *)errorPasscode
{
    ...
    for (int i = 0; i < errorPasscode.length; i++)
    {
        ...
        [self.circleArray[number.intValue] updateCircleState:XXLockCircleStateError];
...
- (void)reset
{
    if (!self.drawing)
    {
        ...
        for (XXLockCircle *circle in self.circleArray)
        {
            [circle updateCircleState:XXLockCircleStateNormal];
        }
...
- (void)updateTrack:(CGPoint)point
{
    ...
    for (XXLockCircle *circle in self.circleArray)
    {
        ...
        {
            if (circle.state == XXLockCircleStateNormal)
            {
                [circle updateCircleState:XXLockCircleStateSelected];
...
- (void)endTrack
{
    ...
    {
        XXLockCircle *circle = self.selectedCircleArray[i];
...
- (void)drawRect:(CGRect)rect
{
    ...
    self.errorPasscode ? [XX_LOCK_COLOR_ERROR set] : [XX_LOCK_COLOR_NORMAL set];
    
    CGPoint addLines[9];
    int count = 0;
    for (XXLockCircle *circle in self.selectedCircleArray)
    {
