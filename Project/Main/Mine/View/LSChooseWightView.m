...
@interface LSChooseWightView()<UIPickerViewDelegate,UIPickerViewDataSource>
...
//高度
@property (nonatomic,assign) int myWeight;
@property (nonatomic,assign) int maxWeight;
@property (nonatomic,assign) int minWeight;
@property (nonatomic,assign) int preWeight;

@end

@implementation LSChooseWightView

+ (void)showWight:(int)preWeight select:(void(^)(NSString *title))selectedBlock
{
    LSChooseWightView *alertView = [[LSChooseWightView alloc]initWithStyle:XWBaseAlertViewStyleBottom];
    alertView.preWeight = preWeight;
...
{
    //7.0英寸到5.4英寸
    return (int)(self.maxWeight - self.minWeight);
}
    ...
    if(!_cancelButton){
        ...
        [_cancelButton setAction:^{
            if(weakself.selectedBlock){
                NSString *str = [NSString stringWithFormat:@"%d",weakself.myWeight];
...
- (void)setPreWeight:(int)preWeight
{
    _preWeight = preWeight;
    if(_preWeight < self.minWeight || _preWeight > self.maxWeight
       ){
        return;
    }
    int row = preWeight - _minWeight;
