...
@property (nonatomic, assign) CGFloat minIntervalWidth; //间隔宽度？
...
- (UIButton *)minSliderBtn {
    if (!_minSliderBtn) {
        ...
        UIImageView *imageView = [[UIImageView alloc]initWithImage:XWImageName(@"slider_image")];
        [_minSliderBtn addSubview:imageView];
        [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.width.height.equalTo(@28);
//            make.center.equalTo(self.minSliderBtn);
            make.edges.equalTo(_minSliderBtn);
        }];
...
- (UIButton *)maxSliderBtn {
    if (!_maxSliderBtn) {
        ...
        UIImageView *imageView = [[UIImageView alloc]initWithImage:XWImageName(@"slider_image")];
        [_maxSliderBtn addSubview:imageView];
        [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.width.height.equalTo(@28);
//            make.center.equalTo(self.maxSliderBtn);
            make.edges.equalTo(_maxSliderBtn);
        }];
    }
    return _maxSliderBtn;
}

- (void)setInitMinValue:(CGFloat)initMinValue
{
    _initMaxValue = initMinValue;
    CGFloat contentWidth = self.width - self.marginCenterX * 2;
    self.minSliderBtn.centerX = initMinValue*contentWidth+self.marginCenterX;
    [self changeLineViewWidth];
}

- (void)setInitMaxValue:(CGFloat)initMaxValue
{
    _initMaxValue = initMaxValue;
    CGFloat contentWidth = self.width - self.marginCenterX * 2;
    self.maxSliderBtn.centerX = _initMaxValue*contentWidth+self.marginCenterX;
    [self changeLineViewWidth];
}
